...
static void tcp_app_thread(void *arg) {
	...
	while (1) {
...
#if (TCP_CLIENT_APP)
		/* Connect to a server */
		IP4_ADDR(&server_address, serverIP_ADDR0, serverIP_ADDR1,
				serverIP_ADDR2, serverIP_ADDR3);
		PRINTF("Client Connection attempt\r\n");
		err = netconn_connect(conn, &server_address, 50002);
		newconn = conn;
#else /*SERVER MODE*/
		/* Grab new connection. */
		...
		if (err == ERR_OK) {
			...
			char data_client_to_server[16]= "que onda loco ma";
			...
			err = netconn_write(newconn,data_with_crc_aes, 26, NETCONN_COPY);
			if (err != ERR_OK) {
...
void tcp_app_init(void) {
	...
	const port_pin_config_t porta4_pin38_config = { kPORT_PullUp, /* Internal pull-up resistor is enabled */
	...
	sys_thread_new("tcp_app_thread", tcp_app_thread, NULL,
